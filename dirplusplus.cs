...
namespace CMD__
{
    public static class dirplusplus
    {
        ...
        public static void Run(string[] args)
        {
            ...
            if (recursive)
            { // If argument "~" is set, work 1 depth recursive
                ...
                foreach (string subdirectory in intelligentSort(Directory.GetDirectories(path), args))
                {
                    ...
                    try
                    {
                        ...
                        foreach (string subsubdirectory in intelligentSort(Directory.GetDirectories(subdirectory), args))
                        {
                            ...
                            { // Otherwise if not permitted, fuck it  ¯\_(ツ)_/¯
        ...
        public static void listFiles(string path, string[] args, bool isSubFile)
        {
            ...
            foreach (string fileName in fileEntries)
            {
                ...
                try
                { // Get FileSize, if System allows it ♥
                    fl = file.Length;
                }
                catch (Exception e)
                { // otherwise just set it to 0. I mean, who cares?
                    fl = 0;
                }
                ...
                { // Otherwise, throw error and say something like "bla, not good" and so on
                ...
                if (args[i].StartsWith("-"))
                {
                    ...
                    { // Searching with patterns
                        if (args.Length > i + 1)
                        {
                            ...
                            { // Is Regular Expression valid? No? This!
                                ConsoleColor OFG = System.Console.ForegroundColor;
                                ConsoleColor OBG = System.Console.BackgroundColor;
                                System.Console.ForegroundColor = ConsoleColor.Yellow;
                                System.Console.BackgroundColor = ConsoleColor.Red;
                            ...
                            { // Search and remove stuff, that does not match.
